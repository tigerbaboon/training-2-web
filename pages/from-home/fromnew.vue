  ...
  <div class="flex justify-center w-full" v-for="(list, x) in lists" :key="x">
    <div class="md:flex w-full md:w-[1600px]">
      <div class="w-full md:w-[500px] border-x shadow-lg">
        ...
        <div class="w-full flex justify-center md:px-2">
          <!-- กรองข้อมูลที่พัก -->
          <div class="w-full" v-if="activeTab === 'fromDetail'">
            ...
            <div class="space-y-3 grid grid-cols-1 mx-4 mb-3">
              <div>
                <label for="ชื่อโครงการ" class="block text-gray-900 text-[18px] md:text-[24px]">ชื่อโครงการ</label>
                <input id="ชื่อโครงการ" type="text" v-model="house_name" placeholder="ระบุชื่อโครงการ..."
                  class="w-full h-[40px] shadow-lg outline-sky-300 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2" />
              </div>
              <div>
                <label for="ประเภทการขาย" class="block text-gray-900 text-[18px] md:text-[24px]">ประเภทการขาย</label>
                <select v-model="sell_type" id="ประเภทการขาย"
                  class="w-full h-[40px] shadow-lg outline-sky-300 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2">
                  <option disabled value="">กรุณาเลือกประเภทการขาย</option>
                  <option v-for="(categoryItem, i) in list.category" :key="i" class="text-black">
                    {{ categoryItem.sell }}
                  </option>
                </select>
              </div>
              <div>
                <label for="ประเภทที่พัก" class="block text-gray-900 text-[18px] md:text-[24px]">ประเภทที่พัก</label>
                <select id="ประเภทที่พัก" v-model="house_type"
                  class="w-full h-[40px] shadow-lg outline-sky-300 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2">
                  <option disabled value="">กรุณาเลือกประเภทที่พัก</option>
                  <option v-for="(categoryItem, i) in list.categoryhome" :key="i" :value="categoryItem.homelist"
                    class="text-black">
              ...
              <div>
                <label for="โซนที่พัก" class="block text-gray-900 text-[18px] md:text-[24px]">โซนที่พัก</label>
                <select id="โซนที่พัก" v-model="zone_id"
                  class="w-full h-[40px] shadow-lg outline-sky-300 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2">
                  <option v-for="(zone, i) in itemsZone" :key="i" :value="i" class="text-black">
                    {{ zone.zone_name }}
                  </option>
                </select>
              </div>
              <div>
                <label for="ชื่อโครงการ" class="block text-gray-900 text-[18px] md:text-[24px]">ราคา</label>
                <input id="ราคา" type="number" min="0" v-model="price" placeholder="ราคาเช่าต่อเดือน"
              ...
              <div class="grid grid-cols-2 gap-x-4 gap-y-2 w-full">
                <div class="">
                  <label for="พื้นที่ใช้สอย (ตร.ม.)"
                    class="block text-gray-900 md:text-[18px] text-[16px]">พื้นที่ใช้สอย(ตร.ม.)</label>
                  <input id="พื้นที่ใช้สอย (ตร.ม.)" v-model="size" type="number" min="0" placeholder="ตร.ม."
                    class="w-full h-[40px] shadow-lg outline-sky-200 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2" />
                </div>
                <div>
                  <label for="ชั้น" class="block text-gray-900 md:text-[18px] text-[16px]">ชั้น</label>
                  <input id="ชั้น" v-model="floor" type="number" min="0" placeholder="จำนวนชัั้น"
                    class="w-full h-[40px] shadow-lg outline-sky-200 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2" />
                </div>
                <div class="">
                  <label for="จำนวนห้องนอน" class="block text-gray-900 md:text-[18px] text-[16px]">จำนวนห้องนอน</label>
                  <input id="จำนวนห้องนอน (ตร.ม.)" v-model="number_of_rooms" type="number" min="0"
                    placeholder="ระบุจำนวนห้อง"
                    class="w-full h-[40px] shadow-lg outline-sky-200 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2" />
                </div>
                <div>
                  <label for="จำนวนห้องน้ำ" class="block text-gray-900 md:text-[18px] text-[16px]">จำนวนห้องน้ำ</label>
                  <input id="จำนวนห้องน้ำ" v-model="number_of_bathrooms" type="number" placeholder="จำนวนห้องนอน"
                    min="0"
                    class="w-full h-[40px] shadow-lg outline-sky-200 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2" />
                </div>
                <!-- <div>
              ...
              <div>
                <label for="product_details"
                  class="block text-gray-900 text-[18px] md:text-[24px]">รายละเอียดบ้านเช่า</label>
                <textarea placeholder="ใส่ข้อมูลบ้านเช่าของคุณ" v-model="description"
                  class="w-full h-[250px] shadow-lg outline-sky-200 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2"
                  id="product_details"></textarea>
              </div>
              <div>
                <label for="address" class="block text-gray-900 text-[18px] md:text-[24px]">ที่อยู่</label>
              ...
              <div class="grid grid-cols-2 gap-x-4 w-full">
                <div>
                  <label for="อัตราค่าไฟ บาท/ต่อหน่วย" class="block text-gray-900 md:text-[18px] text-[16px]">อัตราค่าไฟ
                    บาท/ต่อหน่วย</label>
                  <input id="อัตราค่าไฟ บาท/ต่อหน่วย" v-model="electricity_rate" type="number" min="0"
                    placeholder="ระบุค่าไฟ"
                    class="w-full h-[40px] shadow-lg outline-sky-200 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2" />
                </div>
                <div class="">
                  <label for="อัตราค่าน้ำ บาท/ต่อหน่วย"
                    class="block text-gray-900 md:text-[18px] text-[16px]">อัตราค่าน้ำ บาท/ต่อหน่วย</label>
                  <input id="อัตราค่าน้ำ บาท/ต่อหน่วย" v-model="water_rate" type="number" min="0"
                    placeholder="ระบุค่าน้ำ"
                    class="w-full h-[40px] shadow-lg outline-sky-200 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2" />
                </div>
              </div>
              <div class="w-full">
                <label for="#" class="block text-gray-900 text-[18px] md:text-[24px]">สิ่งอำนวยความสะดวก</label>
                <div class="grid grid-cols-2 gap-2 mt-2">
                  <div v-for="(device, i) in itemsDetail" :key="i">
                    <div class="flex items-center">
                      <input type="checkbox" v-model="item_detail" :value="device.id"
              ...
              <div class="w-full">
                <label for="name" class="block text-gray-900 text-[18px] md:text-[24px]">ชื่อผู้ลงประกาศ</label>
                <input id="name" type="text" v-model="first_name" placeholder="ชื่อ"
                  class="w-full h-[40px] shadow-lg outline-sky-200 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2" />
              </div>
              <div class="w-full">
                <label for="name" class="block text-gray-900 text-[18px] md:text-[24px]">นามสกุลผู้ลงประกาศ</label>
                <input id="name" type="text" v-model="last_name" placeholder="ชื่อ"
                  class="w-full h-[40px] shadow-lg outline-sky-200 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2" />
              </div>
              <div class="grid grid-cols-1 gap-y-3 w-full">
                <div>
                  <label for="phone" class="block text-gray-900 text-[18px] md:text-[24px]">เบอร์ติดต่อ</label>
                  <input id="phone" type="text" v-model="phone_number" placeholder="โปรดระบุ"
                    class="w-full h-[40px] shadow-lg outline-sky-200 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2" />
                </div>
                <div class="">
                  <label for="lineid" class="block text-gray-900 text-[18px] md:text-[24px]">Line ID</label>
                  <input id="lineid" type="text" v-model="line_id" placeholder="โปรดระบุ"
                    class="w-full h-[40px] shadow-lg outline-sky-200 border-[1px] rounded-md border-gray-200 text-[16px] md:text-[20px] px-2" />
                </div>
              </div>
            </div>
          ...
          <!-- อัพโหลดรูป -->
          <div class="w-full md:flex md:flex-col" v-if="activeTab === 'inputImage'">
            <h1 class="text-[22px] md:text-[30px] text-center py-2 font-bold text-gray-800">
              อัพโหลดรูปภาพ
            </h1>
            <div class="w-full flex md:block px-2">
              <div class="w-full md:w-auto border-[2px] border-sky-400 rounded-md md:p-6 m-2">
                ...
                <label
                  ...
                  <input class="text-sm cursor-pointer hidden" type="file" multiple @change="handleSingleImageUpload" />
                  <div class="hidden md:block absolute top-2 left-2 bg-sky-400 text-white px-2 py-1 rounded-tr-lg">
                    รูปหน้าปก
                  </div>
                  <Icon
              ...
              <div class="w-full md:w-auto border-[2px] border-green-400 rounded-md md:p-6 m-2">
                <p class="text-[16px] md:text-[24px] text-center">
                ...
                <label
                  ...
                  <div class="hidden md:block absolute top-2 left-2 bg-green-400 text-white px-2 py-1 rounded-tr-lg">
                    รูปประกอบ
                  </div>
                  <Icon
      ...
      <div class="w-full bg-white mt-3 md:mt-0">
        ...
        <div class="border md:ml-10 mx-4 md:my-3 p-4 rounded-md shadow-lg">
          <div>
            <span class="text-gray-800 text-[16px] md:text-[24px]">ชื่อบ้านเช่า :</span>
            <span class="font-bold text-[18px] md:text-[30px] break-words whitespace-normal block max-w-full" style="
          ...
          <div>
            <span class="text-gray-800 text-[16px] md:text-[24px]">ประเภทการขาย :
            </span>
          ...
          <div>
            <span class="text-gray-800 text-[16px] md:text-[24px]">ประเภทที่พัก :
            </span>
            <span class="font-bold text-[18px] md:text-[30px] break-words">{{
              house_type
            }}</span>
          ...
          <div>
            <span class="text-gray-800 text-[16px] md:text-[24px]">โซนที่พัก :
            </span>
            <span class="font-bold text-[18px] md:text-[30px] break-words">
              {{
          ...
          <div class="flex items-center gap-2">
            <span class="text-gray-800 text-[16px] md:text-[24px]">ราคาเช่า :
            </span>
            <span class="font-bold xl:text-[28px] lg:text-[22px]">{{ formatNumber(price, 0) }}
            </span>
            <div>
              <span v-if="sell_type === 'เช่า'" class="text-gray-800 text-[16px] md:text-[24px]">
                บาท/เดือน
              </span>
              <span v-else-if="sell_type === 'ขาย'" class="text-gray-800 text-[16px] md:text-[24px]">
                บาท
              </span>
            </div>
          </div>
        ...
        <div class="border md:ml-10 mx-4 md:my-3 p-4 rounded-md shadow-lg my-1">
          ...
          <div class="md:flex md:justify-center grid grid-cols-1 gap-1 md:gap-2 md:h-[408px]">
            <div v-if="uploadedImages.length" class="md:w-[710px] relative">
              <button @click="delimg()" class="absolute top-1 right-2 text-rose-600 hover:text-red-500">
              ...
              <div class="w-full h-[200px] md:h-[408px]">
                <img class="w-full h-full rounded-lg object-cover" :src="uploadedImages[0]" alt="Uploaded" />
              </div>
            </div>
            <!-- รูปโชว์ตอนยังไม่ได้เลือกไฟล์รูป -->
            <div v-if="!uploadedImages.length" class="w-full md:w-[710px] md:h-[408px] h-[200px] relative">
              <div
            ...
            <div class="grid grid-cols-1 gap-1 overflow-auto overflow-x-hidden h-[408px]">
              <div v-if="uploadedImages2.length" v-for="(uploadedImages2, index) in uploadedImages2.slice(0)"
                :key="index" class="relative">
                <button @click="delimg2(index)" class="absolute top-1 right-2 text-rose-600 hover:text-red-500">
                ...
                <div class="w-full md:w-[370px] h-[200px] md:h-[200px]">
                  <img class="w-full h-full rounded-lg object-cover" :src="uploadedImages2" alt="Uploaded" />
                </div>
              </div>
              <!-- รูปโชว์ตอนยังไม่ได้เลือกไฟล์รูป -->
        ...
        <div class="border md:ml-10 mx-4 md:my-3 p-4 rounded-md shadow-lg my-1">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 place-items-start">
            <div class="flex items-center justify-center gap-2 h-[40px]">
              <Icon class="text-sky-900 w-[20px] md:w-[30px] h-[20px] md:h-[30px]" name="ion:bed-outline" />
            ...
            <div class="flex items-center justify-center gap-2 h-[40px]">
              <Icon class="text-sky-900 w-[20px] md:w-[30px] h-[20px] md:h-[30px]" name="mdi:bathroom" />
              <p class="text-[16px] md:text-[24px] text-[#2eb840]">
                {{ number_of_bathrooms }}
              </p>
            ...
            <div class="flex items-center justify-center gap-2 h-[40px]">
              <Icon class="text-sky-900 w-[20px] md:w-[30px] h-[20px] md:h-[30px]" name="ri:stairs-line" />
              <p class="text-[16px] md:text-[24px] text-[#2eb840]">
                {{ floor }}
              </p>
            ...
            <div class="flex items-center justify-center gap-2 h-[40px]">
              <Icon class="text-sky-900 w-[20px] md:w-[30px] h-[20px] md:h-[30px]"
        ...
        <div class="border md:ml-10 mx-4 md:my-3 p-4 rounded-md shadow-lg my-1">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 place-items-start">
            <div class="flex items-center justify-center gap-2 h-[40px]">
              <Icon class="text-sky-900 w-[20px] md:w-[30px] h-[20px] md:h-[30px]" name="iconoir:light-bulb-on" />
        ...
        <div class="border md:ml-10 mx-4 md:my-3 p-4 rounded-md shadow-lg my-1">
          <label for="inputtext" class="text-[16px] md:text-[24px]">รายละเอียดบ้านเช่า :
          </label>
        ...
        <div class="border md:ml-10 mx-4 md:my-3 p-4 rounded-md shadow-lg my-1">
          <label for="inputaddress" class="text-[16px] md:text-[24px]">ที่อยู่ :
          </label>
        ...
        <div class="border md:ml-10 mx-4 md:my-3 p-4 rounded-md shadow-lg my-1">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-y-[1px] md:gap-y-3" v-if="item_detail.length">
            <li class="text-[16px] md:text-[22px] custom-bullet" v-for="device in item_detail" :key="device">
              &nbsp;{{
        ...
        <div class="border md:ml-10 mx-4 md:my-3 p-4 rounded-md shadow-lg my-1">
          ...
          <div class="grid grid-cols-1 gap-2 my-1">
            <div class="flex items-center gap-1 md:gap-2">
              <Icon class="text-sky-900 w-[20px] md:w-[30px] h-[20px] md:h-[30px]" name="icon-park-solid:edit-name" />
              <p class="text-[16px] md:text-[24px]">ชื่อ :</p>
              <p class="text-[16px] md:text-[24px] text-[#2eb840] break-words whitespace-pre-line">
                {{ first_name }}
              </p>
            </div>
            <div class="flex items-center gap-1 md:gap-2">
              <Icon class="text-sky-900 w-[20px] md:w-[30px] h-[20px] md:h-[30px]" name="icon-park-solid:edit-name" />
              <p class="text-[16px] md:text-[24px]">นามสกุล :</p>
              <p class="text-[16px] md:text-[24px] text-[#2eb840] break-words whitespace-pre-line">
                {{ last_name }}
              </p>
            </div>
            <div class="flex items-center gap-1 md:gap-2">
              ...
              <p class="text-[16px] md:text-[24px]">เบอร์โทร :</p>
              <p class="text-[16px] md:text-[24px] text-[#2eb840] break-words whitespace-pre-line">
                {{ phone_number }}
              </p>
            </div>
            <div class="flex items-center gap-1 md:gap-2">
              <Icon class="text-sky-900 w-[20px] md:w-[30px] h-[20px] md:h-[30px]" name="ri:line-line" />
              <p class="text-[16px] md:text-[24px]">ID Line :</p>
              <p class="text-[16px] md:text-[24px] text-[#2eb840] break-words whitespace-pre-line">
                {{ line_id }}
              </p>
            </div>
...
const formattedMessage = computed(() => {
  return (description.value = description.value.replace(/<br\s*\/?>/g, "\n")); // กลับ `<br>` เป็น `\n` ก่อนส่งไปจัดการต่อ
});
...
const onSubmit = async () => {
  ...
  try {
    ...
    const resp: any = await service.createHouse.createhome(formdata);
...
const getitem = async () => {
  try {
    const resp: any = await service.createHouse.listzone(query.value);
    if (resp.Code == "200") {
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: `Unexpected response code: ${resp.Code}`,
      });
    }
  } catch (error: any) {
...
const getdetail = async () => {
  try {
    const resp: any = await service.createHouse.listamenity(query.value);
    if (resp.Code == "200") {
...
function updateMap(): void {
  ...
  g_modal.value.location_latitute = lat;
  g_modal.value.location_longitute = lng;
}
