        ...
        <div class="border md:ml-10 mx-4 md:my-3 p-4 rounded-md shadow-lg">
          <div>
            ...
            <span
              ...
              style="
                ...
                overflow: hidden;
              "
              >{{ getHouse.datas.house_name }}</span
            >
          </div>
          <div>
            <span class="text-gray-800 text-[16px] md:text-[24px]"
          ...
          <div class="flex items-center gap-2">
            ...
            <span class="font-bold xl:text-[28px] lg:text-[22px]"
              >{{ formatNumber(getHouse.datas.price, 0) }}
            </span>
            <div>
              <span
        ...
        <div class="border md:ml-10 mx-4 md:my-3 p-4 rounded-md shadow-lg my-1">
          ...
          >
            ...
            <!-- <div class="grid grid-cols-1 gap-1 overflow-auto overflow-x-hidden h-[408px]">
              <div v-if="newImages.length || getHouse.datas.images.length">
               
                <div v-for="(newImage, index) in newImages" :key="'new-' + index" class="relative">
                  <button @click="deleteImage(newImages, index)"
                  ...
                  <div class="w-full md:w-[370px] h-[200px] md:h-[200px]">
                    <img class="w-full h-full rounded-lg object-cover" :src="newImage" alt="Uploaded" />
                  </div>
                </div>
                
                <div v-for="(image, index) in getHouse.datas.images" :key="'old-' + index" class="relative">
                  <button @click="deleteImage(getHouse.datas.images, index)"
                    class="absolute top-1 right-2 text-rose-600 hover:text-red-500">
                    <Icon class="w-[20px] h-[20px]" name="mdi:close-thick" />
                  </button>
                  <span
                  ...
                  <div class="w-full md:w-[370px] h-[200px] md:h-[200px]">
                    <img class="w-full h-full rounded-lg object-cover" :src="image.url" alt="Uploaded" />
                  </div>
                </div>
              </div>
...
const updateByHome = async (id: string) => {
  try {
    ...
    const resp: any = await service.listhome.updateByEditHome(formdata, id);
    if (resp.Code == "200") {
      await Swal.fire({
        icon: "success",
        title: "แก้ไขสำเร็จ",
...
const getitem = async () => {
  try {
    const resp: any = await service.travel.zone(query.value);
    if (resp.Code == "200") {
